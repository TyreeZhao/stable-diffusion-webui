...
def upscaler_to_index(name: str):
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid upscaler, needs to be one of these: {' , '.join([x.name for x in sd_upscalers])}")
...
def script_name_to_index(name, scripts):
    try:
        ...
        raise HTTPException(
            status_code=422, detail=f"Script '{name}' not found")
...
class Api:
    def __init__(self, app: FastAPI, queue_lock: Lock):
        ...
        self.add_api_route("/sdapi/v1/extra-single-image", self.extras_single_image_api,
                           methods=["POST"], response_model=ExtrasSingleImageResponse)
        self.add_api_route("/sdapi/v1/extra-batch-images", self.extras_batch_images_api,
                           methods=["POST"], response_model=ExtrasBatchImagesResponse)
        self.add_api_route("/sdapi/v1/png-info", self.pnginfoapi,
                           methods=["POST"], response_model=PNGInfoResponse)
        self.add_api_route("/sdapi/v1/progress", self.progressapi,
                           methods=["GET"], response_model=ProgressResponse)
        ...
        self.add_api_route("/sdapi/v1/create/embedding", self.create_embedding,
                           methods=["POST"], response_model=CreateResponse)
        ...
        self.add_api_route("/sdapi/v1/scripts", self.get_scripts_list,
                           methods=["GET"], response_model=ScriptsList)
        self.add_api_route("/invocations", self.invocations, methods=["POST"], response_model=Union[TextToImageResponse,
                           ImageToImageResponse, ExtrasSingleImageResponse, ExtrasBatchImagesResponse, InvocationsErrorResponse])
        self.add_api_route("/ping", self.ping,
                           methods=["GET"], response_model=PingResponse)
    ...
    def get_scripts_list(self):
        t2ilist = [str(title.lower())
                   for title in scripts.scripts_txt2img.titles]
        i2ilist = [str(title.lower())
                   for title in scripts.scripts_img2img.titles]
    ...
    def extras_batch_images_api(self, req: ExtrasBatchImagesRequest):
        ...
        with self.queue_lock:
            result = postprocessing.run_extras(
                extras_mode=1, image="", input_dir="", output_dir="", save_output=False, **reqDict)
    ...
    def invocations(self, req: InvocationsRequest):
        ...
        try:
            ...
            if hypernetwork_s3uri != '':
                self.download_s3files(hypernetwork_s3uri, os.path.join(
                    script_path, shared.cmd_opts.hypernetwork_dir))
            ...
            if req.task == 'text-to-image':
                if embeddings_s3uri != '':
                    print("embeddings_s3uri is :", embeddings_s3uri)
                    print("shared.cmd_opts.embeddings_dir",
                          shared.cmd_opts.embeddings_dir)
                    self.download_s3files(embeddings_s3uri, os.path.join(
                        script_path, shared.cmd_opts.embeddings_dir))
                    sd_hijack.model_hijack.embedding_db.load_textual_inversion_embeddings()
                response = self.text2imgapi(req.txt2img_payload)
                response.images = self.post_invocations(
                    response.images, quality)
                return response
            elif req.task == 'image-to-image':
                if embeddings_s3uri != '':
                    print("embeddings_s3uri is ",embeddings_s3uri)
                    print("shared.cmd_opts.embeddings_dir is ", shared.cmd_opts.embeddings_dir)
                    self.download_s3files(embeddings_s3uri, os.path.join(
                        script_path, shared.cmd_opts.embeddings_dir))
                    sd_hijack.model_hijack.embedding_db.load_textual_inversion_embeddings()
                ...
                response = self.extras_single_image_api(
                    req.extras_single_payload)
                response.image = self.post_invocations(
                    [response.image], quality)[0]
                return response
            elif req.task == 'extras-batch-images':
                response = self.extras_batch_images_api(
                    req.extras_batch_payload)
                response.images = self.post_invocations(
                    response.images, quality)
                return response
            else:
        ...
        for obj in objs:
            ...
            if obj_key not in cache or cache[obj_key] != response['ETag']:
                ...
                s3_client.download_file(
                    bucket, obj.key, os.path.join(path, filename))
